<script>
	...
	// export let bilgi_template = 'asdads';
	// /** @type {Array<string>} */
	/**
	 * @type {any[]}
	 */
...
<div class="pick_categories">
	...
	{#each categories as item}
		<div class="relative rounded border border-gray-200 dark:border-gray-700">
			<!-- <Checkbox on:click={loggg} value={item.name} bind:group={category_list} class="w-full p-4"
				>{item.name}</Checkbox
			> -->
			<label>
				<input
					...
					name="flavours"
					value={item.name}
					bind:group={$contact_info.categories}
					on:click={loggg}
				/>
				{item.name}
				<!-- {category_list} -->
		...
		> -->
			<label>
				<input
					...
					name="flavours"
					value={item.name}
					bind:group={$contact_info.products}
					on:click={loggg}
				/>
				{item.name}
				<!-- {category_list} -->
